...
bAddIncome = Button(toolbar, text="Add Income")
...
def update_profile():
    ...
    # gen = eGender.get()
    query = "UPDATE user SET username = %s,password = %s,name = %s,age = %s,email = %s,address = %s "
    try:
        con = mysql.connect(host="localhost",user="root",password="",database="exptrack")
        cur = con.cursor()
        params = (username,password,name,str(age),email,address,)
